...
# --- Parameters
outfile      = 'Main.out'
plotChannels = ['Wind1VelX','RotSpeed','BldPitch1','OoPDefl1','TTDspFA','RtAeroPwr']

# --- Read out file
# NOTE: it's best to use FASTInputFile from the python-toolbox instead:
#    from pyFAST.input_output.fast_input_file import FASTInputFile
#    df = FASTInputFile(outfile).toDataFrame()
...
fig.subplots_adjust(left=0.12, right=0.95, top=0.98, bottom=0.06, hspace=0.20, wspace=0.20)
...
fig.savefig(os.path.join(os.getcwd(), '_results', 'PlotSim.png'))
plt.show()
